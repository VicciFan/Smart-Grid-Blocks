...
% pi-section equivalent of regulator
pi_circuits(k_reg).Y_ij = t_reg * Y_reg;
pi_circuits(k_reg).Y_i_ij = t_reg * (t_reg-1) * Y_reg;
pi_circuits(k_reg).Y_j_ij = (1-t_reg) * Y_reg;
...
% pi-section equivalent of regulator
pi_circuits(k_reg).Y_ij = Y_reg;
pi_circuits(k_reg).Y_i_ij = 0;
pi_circuits(k_reg).Y_j_ij = 0;

% base change in downstream system
for k=k_reg:n_lines
    pi_circuits(k).Y_ij = pi_circuits(k).Y_ij * t_reg^2;
    pi_circuits(k).Y_i_ij = pi_circuits(k).Y_i_ij * t_reg^2;
    pi_circuits(k).Y_j_ij = pi_circuits(k).Y_j_ij * t_reg^2;
end
