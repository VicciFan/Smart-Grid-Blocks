...
for i=1:length(dp)-N    
    ...
    Tl = Tlweek(i+1:i+N);
    Th = Thweek(i+1:i+N);
    Tmean= 0.5*(Tlweek+Thweek);
    nonprice =  tariffweek(i+1:i+N);
    nonprice(:)=0;
    
    if (0.5*Tmean(i)<= x_init)
        if x_init<=(0.5*Tmean(i)+0.5)
        u =max_flow.*(T_init*ones(3,1)-x_init(1:3));
        end
    else
        u=[0;0;0];
    end
    
    if ((Tstmax-20) <= T_init) && (T_init<= (Tstmax-10))
        ust =stmax;
    else
        ust=0;
    end
    ...
    costThermo = costThermo + ust*tariff(1);

    % actual disturbance
    dp_true = dplow(i, :)';
    
    % Complete the code here...
    xplus = A*x_init+Bu*u(:,1)+Bd*dp_true;
    T_plus = Ast*T_init+Bust*[u(:,1);ust]+Bdst*dp_true;
    x_init = xplus;
    T_init = T_plus;
    ...
    % Update the history
    StateHistory = [StateHistory, xplus];
    InputHistory = [InputHistory, [u(:,1); ust]];
    StorageHistory = [StorageHistory,T_plus];
end
