...
for k=1:N
    ...
    x = A*x+Bu*u(:,k)+Bd*d(:,k);
    Tst = Ast*Tst+Bust*[u(:,k);st(k)]+Bdst*d(:,k);    % use prediction of disturbance
    constraints = [constraints, ep(:,k) >= Tlow(k)*ones(3,1)-x(1:nu); ep(:,k) >= x(1:nu)-Thigh(k)*ones(3,1)];
    constraints = [constraints, Tst <= Tstmax];
    objective = objective  +  Sep*norm(ep(:,k),1) + Sinp*normInp*tf(:,k)*st(:,k);  %% why normInp??
end
...
for i=1:length(dp)-N 
    ...
    % ***************************************************
    pred = dp(i:i+N-1,:)';
    tariff =  tariffweek(i+1:i+N);   %why is it 2-25?
    Tl = Tlweek(i+1:i+N);
    Th = Thweek(i+1:i+N);
...
for i=1:length(dp)-N    
    ...
    % Update the history
    StateHistory = [StateHistory, xplus];
    InputHistory = [InputHistory, [u; ust]];
    StorageHistory = [StorageHistory,T_plus];
end
